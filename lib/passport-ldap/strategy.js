...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (!req.body || !req.body[self._options.usernameField] || !req.body[self._options.passwordField]) {
    if(self._options.debug) console.log('(EE) [ldapjs] no username or password on req.body expected fields', {username: self._options.usernameField, password: self._options.passwordField});
    ...
    if (self._options.authOnly) {
      ...
      if (self._options.authMode !== 1) {
        ...
        if(typeof self._options.base !== 'string'){
          ...
          if (self._options.nameFormatter) {
            var name = self._options.nameFormatter(username);
          } else {
            var name = username;
          }
        }
      ...
      if (self._options.debug) console.log('(EE) [ldapjs] searching for:', search.filter);
      Client.search(dn, search, function (err, res) {
        ...
        res.on('searchEntry', function(entry) {
          ...
          self._verify(profile, function(err, user) {
            ...
            if (!user) {
              var retCode = self._challenge(username);
              if (self._options.debug) console.log('(EE) [ldapjs] LDAP user error:', retCode);
              return self.fail(retCode);
            }
